...
ggplot(data=var_avg_slope, aes(x=timestamp, y=stat)) + geom_line() + ylab("SD Average Slope") + xlab("Time") +
  geom_hline(yintercept=7)

colnames(var_avg_slope) <- c("dt", "stat")

sd_var_avg_slope <- intervalAnalysis(var_avg_slope,
                                     sd,
                                     "stat",
                                     windowLength=10,
                                     windowStep=1)
colnames(sd_var_avg_slope) <- c("dt", "stat")

ggplot(data=sd_var_avg_slope, aes(x=dt, y=stat)) + geom_line() + geom_hline(yintercept=2.0)
```
...
c <-  7.0 #medianCalc(var_avg_slope$stat,propDaylight1)  #threshold value (critical value)
...
zero_dist <- vector("numeric")

for(i in 1:(length(var_avg_slope$distC)-1)) {
  if(var_avg_slope$distC[i]<0 & var_avg_slope$distC[i+1]>0) {
    zero_dist <- rbind(zero_dist, c(i))
  }
}

var_avg_slope$dt[zero_dist[1]]
var_avg_slope$dt[zero_dist[length(zero_dist)]]

ggplot(data=var_avg_slope, aes(x=dt, y=stat)) + geom_line() + ylab("SD Average Slope") + xlab("Time") + geom_vline(xintercept=var_avg_slope$dt[zero_dist[1]])
